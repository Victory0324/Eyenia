...
export default function List({ navigation }) {
  ...
  function filterData(data) {
    return data.filter(
      function (item) {
        if (item.vehicleName.toLowerCase().includes(searchText.toLowerCase())) {
          return item;
        }
        if (item.deviceImei.toLowerCase().includes(searchText.toLowerCase())) {
          return item;
        }
      }
    );
  ...
  const handleDeleteVehicle = (imei) => {
    ...
    dispatch(deleteVehicle(userReducer.token, imei,navigation));
    // Alert.dismiss();
  };
  useFocusEffect(
    React.useCallback(() => {
      dispatch(vehicleList(userReducer.token, userReducer.user.id));
    }, [])
  );
  ...
  const VehicleItem = (item) => {
    ...
    let diffInMin = 0;
    const teltonika = item.teltonikas[0];
    if (item.teltonikas.length > 0) {
      ...
      <View style={styles.cardview}>
        <View style={{ flexDirection: 'row', marginTop: 2, paddingHorizontal: 35, justifyContent: "space-between" }}>
          <View style={{ flexDirection: 'row', marginTop: 12, alignItems: 'center' }}>
            ...
            <Text style={{ fontSize: 14, fontWeight: '600', color: '#18567F', marginLeft: 10 }}>{item.vehicleName}</Text>
          </View>
          <View style={{ flexDirection: 'row', marginTop: 12, alignItems: 'center' }}>
            <Image source={require('../../assets/battery.png')} style={styles.icon} />
            <Text style={{ fontSize: 10, fontWeight: '400', marginLeft: 10 }}>{teltonika ? teltonika.battery : 0}%</Text>
          </View>
        </View>
        ...
        <View style={{
          ...
          <View style={{ flexDirection: 'row', marginTop: 16, justifyContent: 'space-between' }}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: wp('12%'), alignItems: 'center' }}>
              ...
              <Text style={{ fontSize: 10, fontWeight: '400', }}>{0}L</Text>
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: wp('12%'), alignItems: 'center' }}>
              <Image source={require('../../assets/network.png')} style={styles.icon} />
              <Text style={{ fontSize: 10, fontWeight: '400', }}>{teltonika ? signals[teltonika.signal] : "None"}</Text>
            </View>
          </View>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <Text style={{ flex: 0.9, fontSize: 9, fontWeight: '400', lineHeight: 12 }}>
              {item.teltonikas[0] ? item.teltonikas[0].address : 'no address information'}
            </Text>
            <View style={{ flex: 0.1, flexDirection: 'row', padding: 5, alignItems: 'center', }}>
              ...
              >
                <MenuTrigger onPress={() => setSelectedItem(item)}>
                  <View>
                    ...
                    onSelect={() => {
                      ...
                      Alert.alert(
                        ...
                        [
                          {
                            text: 'Yes',
                            onPress: () => handleDeleteVehicle(item.deviceImei),
                          },
                          {
                            text: 'No',
  ...
  const VehicleList = () => {
    ...
    if (!vehicleReducer.isVehicleList && vehiclesList.length === 0) {
      return (
