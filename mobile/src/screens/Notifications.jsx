...
import { SafeAreaView, Text, TouchableOpacity, View, StatusBar, StyleSheet, Slider, TextInput } from 'react-native'
...
function Notifications({ navigation }) {
    ...
    useFocusEffect(
        React.useCallback(() => {
            dispatch(showNotifications(userReducer.token, userReducer.user.id));
        }, [])
    );

    const NotificationList = () => {
        if (!notificationsReducer.isGettingNotifies && notificationsReducer.length === 0) {
            return (
    ...
    const renderNotificationItem = (item) => {
        return (
            <View style={styles.listItem}>
                ...
                <Text style={styles.listString}>item.time</Text>
            </View>
        )
    }
    ...
    return (
        <SafeAreaView style={{ marginTop: 20, }}>
            ...
            <View style={{ flexDirection: 'row', width: wp('90%'), height: hp('6.5%'), borderRadius: 4, marginLeft: 10, marginTop: 5, backgroundColor: "#18567F12", alignItems: 'center' }}>
                <TextInput placeholder='Search Vehicle' style={{ width: '100%', paddingLeft: 10 }} />
            </View>
